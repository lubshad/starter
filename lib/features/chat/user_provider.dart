...
import 'package:agora_chat_uikit/sdk_service/chat_sdk_define.dart';
import 'package:agora_chat_uikit/sdk_service/observers/group_observer.dart';
...
class _UserProviderWidgetState extends State<UserProviderWidget>
  ...
  void onGroupNameChangedByMeSelf(Group group) {
    ...
    profile =
        profile?.copyWith(showName: group.name) ??
  ...
  Future<void> loadGroupInfos() async {
    ...
    List<ChatUIKitProfile> profiles = groups
        .map(
          (e) => ChatUIKitProfile.group(
            id: e.groupId,
            groupName: e.name,
            avatarUrl: e.extension,
          ),
        )
        .toList();
